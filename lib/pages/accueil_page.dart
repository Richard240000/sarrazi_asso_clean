...
class _AccueilPageState extends State<AccueilPage> {
  ...
  Future<int?> getLatestNewsId() async {
    try {
      ...
      // On reste silencieux pour ne pas gêner l'utilisateur sur la page d'accueil.
    }
  ...
  Future<void> _openAlerteSignalement() async {
    ...
    await Navigator.push(context, MaterialPageRoute(builder: (context) => const AlerteSignalementPage()));
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      Expanded(
                        child: _buildTile(label: 'Alertes &\nSignalements', icon: Icons.notifications_active, color: const Color(0xFF1A237E), onTap: _openAlerteSignalement, badgeVisible: hasNewAlert),
                      ),
                      Expanded(child: SizedBox.shrink()),
                    ],
                  ),
  ...
  Widget _buildBandeauMaj() {
    ...
    final String message = bandeauMaj!['message']?.toString() ?? 'Nouvelle version disponible';
    String buttonUrl = '';
  ...
  Widget _buildTile({required String label, required IconData icon, required Color color, required VoidCallback onTap, bool isSecure = false, bool badgeVisible = false}) {
    return GestureDetector(
      onTap: onTap,
      child: Badge(
        smallSize: 15,
