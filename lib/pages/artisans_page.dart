...
class _ArtisansPageState extends State<ArtisansPage> {
  ...
  Future<void> chargerArtisans() async {
    ...
    if (response.isSuccess) {
      setState(() {
        ...
        artisans = artisans.toList().sortedBy((x) => (x['intitule'].toString().contains(':') ? x['intitule'].split(':')[1] : x['intitule']).toString().toLowerCase().trimLeft());
      });
    } else {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    return BasePage(title: "Artisans", body: getBody(), message: "Si vous souhaitez ajouter ou modifier votre activité, n'hésitez pas à nous contacter !", withContact: true);
  }
              ...
              return Card(
                ...
                child: ListTile(
                  ...
                  leading: Container(
                    ...
                    child: Text(
                      (artisan['intitule'].toString().contains(':') ? artisan['intitule'].split(':')[1] : artisan['intitule']).trim().substring(0, 1).toUpperCase(),
                      textAlign: TextAlign.center,
                      style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 20),
                    ),
                  ),
                  title: Text(titreArtisan, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                    ...
                    children: [
                      ...
                      Align(
                        ...
                        child: ElevatedButton.icon(
                          ...
                          style: ElevatedButton.styleFrom(backgroundColor: Colors.white, surfaceTintColor: Color(0xFF00838F), foregroundColor: Color(0xFF00838F)),
                        ),
                      ),
                    ],
                  ),
